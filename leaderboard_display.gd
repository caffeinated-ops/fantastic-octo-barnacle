...
func display_leaderboard():
	...
	if data.size() == 0:
		...
		no_entries_label.add_theme_font_override("font", load("res://fonts/Xolonium-Regular.ttf"))
	...
	for i in range(min(data.size(), 10)):
		...
		entry_label.add_theme_font_override("font", load("res://fonts/Xolonium-Regular.ttf"))
