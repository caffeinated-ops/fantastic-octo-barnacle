...
func _process(delta: float) -> void:
	...
	# Handle attack input
	if Input.is_action_just_pressed("attack") and attack_cooldown <= 0 and attack_delay_timer <= 0 and not attack_active:
		attack_delay_timer = ATTACK_DELAY
	
	# Update HUD with attack status
	get_node("/root/Main/HUD").update_attack_status(attack_cooldown, attack_active)
